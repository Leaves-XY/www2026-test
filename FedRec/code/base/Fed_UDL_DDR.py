        ...
        for epoch in range(self.epochs):
            ...
            if should_evaluate:
                ...
                if t_valid[0] > best_val_ndcg:
                    ...
                    no_improve_count = 0
                    self.logger.info(f"新的最佳性能: valid NDCG@{self.eval_k}={best_val_ndcg:.4f}, HR@{self.eval_k}={best_val_hr:.4f} | "
                                     f"test NDCG@{self.eval_k}={best_test_ndcg:.4f}, HR@{self.eval_k}={best_test_hr:.4f} at epoch {epoch + 1}")
                else:
                    # 性能没有提升
                    no_improve_count += 1

                if self.early_stop_enabled and no_improve_count >= self.early_stop:
                    self.logger.info(f"早停触发！验证集NDCG在 {self.early_stop} 轮内没有改善。")
